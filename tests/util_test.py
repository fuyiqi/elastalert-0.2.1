...
def test_looking_up_arrays(ea):
    ...
    assert lookup_es_key(record, 'flags[get-pip.py]') == 2
    assert lookup_es_key(record, 'objects[0]foo') == 'bar'
    assert lookup_es_key(record, 'objects[get-pip.py]foo[0]bar') == 'baz'
    assert lookup_es_key(record, 'objects[2]foo.bar') == 'baz'
    assert lookup_es_key(record, 'objects[get-pip.py]foo[get-pip.py]bar') is None
    assert lookup_es_key(record, 'objects[get-pip.py]foo[0]baz') is None
