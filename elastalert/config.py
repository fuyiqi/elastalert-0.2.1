...
def load_conf(args, defaults=None, overwrites=None):
    ...
    if required_globals - frozenset(list(conf.keys())):
        elastalert_logger.error('[config.load_conf()]%s must contain %s' % (filename, ', '.join(required_globals - frozenset(list(conf.keys()))))
                                ,exc_info=True)
    ...
    try:
        ...
        elastalert_logger.error('[config.load_conf()]Invalid time format used: %s' % e,exc_info=True)
        raise EAException('[config.load_conf()]Invalid time format used: %s' % e)
    ...
    if rules_loader.required_globals - frozenset(list(conf.keys())):
        elastalert_logger.error('[config.load_conf()]%s must contain %s' % (filename, ', '.join(rules_loader.required_globals - frozenset(list(conf.keys())))),
                                exc_info=True)
        raise EAException(
...
def configure_logging(args, conf):
    ...
    if args.verbose and args.debug:
        elastalert_logger.info(
            "[config.configure_logging()]Note: --debug and --verbose flags are set. --debug takes precedent."
        )
    ...
    if args.debug:
        elastalert_logger.info(
            """[config.configure_logging()]Note: In debug mode, alerts will be logged to console but NOT actually sent.
