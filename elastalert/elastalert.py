...
class ElastAlerter(object):
    ...
    def __init__(self, args):
        ...
        if self.verbose and self.debug:
            elastalert_logger.info(
                "[elastalert.ElastAlerter.__init__()]Note: --debug and --verbose flags are set. --debug takes precedent."
            )
        ...
        if self.debug:
            elastalert_logger.info(
                """[elastalert.ElastAlerter.__init__()]Note: debug模式, alert日志会输出到控制台,但不会发送.
                要发送告警又留日志, 请用 --verbose."""
            )
        ...
        if self.args.es_debug_trace:
            ...
            '''
            elastalert_logger.info("[elastalert.ElastAlerter.__init__()]self.args.es_debug_trace="+str(self.args.es_debug_trace))
            tracer = elastalert_logger
        ...
        elastalert_logger.info(msg='[elastalert.__init__()]Rules loaded num='+str(len(self.rules)))
    ...
    def get_index(rule, starttime=None, endtime=None):
        ...
        if rule.get('use_strftime_index'):
            if starttime and endtime:
                index1 = format_index(index, starttime, endtime, add_extra)
                elastalert_logger.info("[elastalert.ElastAlerter.get_index()]index1= %s" % (index1))
                ...
                index2 = index[:format_start] + '*' + index[format_end:]
                elastalert_logger.info("[elastalert.ElastAlerter.get_index()]index2= %s" % (index2))
                return index2
        else:
            index3 = index
            elastalert_logger.info("[elastalert.ElastAlerter.get_index()]index3= %s" % (index3))
            return index3
    ...
    def get_query(filters, starttime=None, endtime=None, sort=True, timestamp_field='@timestamp', to_ts_func=dt_to_ts, desc=False,
        ...
        #elastalert_logger.info('[elastalert.ElastAlerter.get_query()]query='+str(query))
        return query
    ...
    def get_terms_query(self, query, rule, size, field, five=False):
        ...
        if not five:
            ...
            aggs_query['aggs'] = {'counts': {'terms': {'field': field,
                                                       'size': size,
                                                       'min_doc_count': rule.get('min_doc_count', 1)}}}
        elastalert_logger('[elastalert.ElastAlerter.get_terms_query()]aggs_query='+str(aggs_query))
        return aggs_query
    ...
    def get_aggregation_query(self, query, rule, query_key, terms_size, timestamp_field='@timestamp'):
        ...
        elastalert_logger.info('[elastalert.ElastAlerter.get_aggregation_query()]aggs_query=' + str(aggs_query))
        return aggs_query
    ...
    def get_index_start(self, index, timestamp_field='@timestamp'):
        ...
        try:
            ...
            elastalert_logger.error("[elastalert.ElastAlerter.get_index_start()]es query error= %s" % (e), {'index': index, 'query': query},
                                    exc_info=True)
            self.handle_error("[elastalert.ElastAlerter.get_index_start()]es query error= %s" % (e), {'index': index, 'query': query})
            return '1969-12-30T00:00:00Z'
        if len(res['hits']['hits']) == 0:
            # Index is completely empty, return a date before the epoch
            return '1969-12-30T00:00:00Z'
        res_timestamp = res['hits']['hits'][0][timestamp_field]
        elastalert_logger.info("[elastalert.ElastAlerter.get_index_start()]res_timestamp= %s" % (res_timestamp))
    ...
    def process_hits(rule, hits):
        ...
        for hit in hits:
            ...
            if not ts and not rule["_source_enabled"]:
                elastalert_logger.error("[elastalert.ElastAlerter.process_hits()]Error: No timestamp was found for hit."
                                        " '_source_enabled' is set to false, check your mappings for stored fields",
                                        exc_info=True)
                raise EAException(
                    "[elastalert.ElastAlerter.process_hits()]Error: No timestamp was found for hit. "
                    "'_source_enabled' is set to false, check your mappings for stored fields"
                )
        ...
        #elastalert_logger.info("[elastalert.ElastAlerter.process_hits()]num(processed_hits)="+str(len(processed_hits)))
    ...
    def get_hits(self, rule, starttime, endtime, index, scroll=False):
        ...
        try:
            ...
            elastalert_logger.debug("[elastalert.ElastAlerter.get_hits()]res="+str(res))
        except ElasticsearchException as e:
            # Elasticsearch sometimes gives us GIGANTIC error messages
            # (so big that they will fill the entire terminal buffer)
            if len(str(e)) > 1024:
                e = str(e)[:1024] + '... (%d characters removed)' % (len(str(e)) - 1024)
            elastalert_logger.error('[elastalert.ElastAlerter.get_hits()]Error running query: %s'
                                    % (e), {'rule': rule['name'], 'query': query},
                                    exc_info=True)
            self.handle_error('[elastalert.ElastAlerter.get_hits()]Error running query: %s'
                              % (e), {'rule': rule['name'], 'query': query})
            return None
        hits = res['hits']['hits']
        self.thread_data.num_hits += len(hits)
        lt = rule.get('use_local_time')
        status_log = "已请求rule= %s;\n\tpretty(开始时间)=\t%s;\n\tpretty(结束时间)=\t%s;\n\tnum_hits/hits= %s/%s;" % (
            rule['name'],
        ...
        if self.thread_data.total_hits > rule.get('max_query_size', self.max_query_size):
            elastalert_logger.info("[elastalert.ElastAlerter.get_hits()]%s (scrolling..)" % status_log)
        else:
            elastalert_logger.info("[elastalert.ElastAlerter.get_hits()]"+status_log)
        ...
        elastalert_logger.info("[elastalert.ElastAlerter.get_hits()]"
                               "当query_body= %s;\n\t那么num(hits)= %s"%(query,len(hits)))
        return hits
    ...
    def get_hits_count(self, rule, starttime, endtime, index):
        ...
        try:
            ...
            if len(str(e)) > 1024:
                e = str(e)[:1024] + '... (%d characters removed)' % (len(str(e)) - 1024)
            elastalert_logger.error('[elastalert.ElastAlerter.get_hits_count()]Error running count query: %s' % (e), {'rule': rule['name'], 'query': query},
                                    exc_info=True)
            self.handle_error('[elastalert.ElastAlerter.get_hits_count()]Error running count query: %s' % (e), {'rule': rule['name'], 'query': query})
            return None

        self.thread_data.num_hits += res['count']
        lt = rule.get('use_local_time')
        elastalert_logger.info(
            "[elastalert.ElastAlerter.get_hits_count()]已请求rule= %s;\n\tpretty(开始时间)=\t%s;\n\tpretty(结束时间)=\t%s;\n\tnum(hits)= %s"
    ...
    def get_hits_terms(self, rule, starttime, endtime, index, key, qk=None, size=None):
        ...
        try:
            ...
            if len(str(e)) > 1024:
                e = str(e)[:1024] + '... (%d characters removed)' % (len(str(e)) - 1024)
            elastalert_logger.error('[elastalert.ElastAlerter.get_hits_terms()]Error running terms query: %s' % (e), {'rule': rule['name'], 'query': query},
                                    exc_info=True)
            self.handle_error('[elastalert.ElastAlerter.get_hits_terms()]Error running terms query: %s' % (e), {'rule': rule['name'], 'query': query})
            return None
        ...
        lt = rule.get('use_local_time')
        elastalert_logger.info(
            '[elastalert.ElastAlerter.get_hits_terms()]已请求rule= %s;\n\tpretty(开始时间)=\t%s;\n\tpretty(结束时间)=\t%s;\n\tnum(buckets)= %s '
    ...
    def get_hits_aggregation(self, rule, starttime, endtime, index, query_key, term_size=None):
        ...
        try:
            ...
            if len(str(e)) > 1024:
                e = str(e)[:1024] + '... (%d characters removed)' % (len(str(e)) - 1024)
            elastalert_logger.error('[elastalert.ElastAlerter.get_hits_aggregation()]Error running query: %s' % (e), {'rule': rule['name']},
                                    exc_info=True)
            self.handle_error('[elastalert.ElastAlerter.get_hits_aggregation()]Error running query: %s' % (e), {'rule': rule['name']})
            return None
        if 'aggregations' not in res:
            return {}
        if not rule['five']:
        ...
        try:
            ...
            if res['hits']['hits']:
                ...
                if ts_now() - endtime < self.old_query_limit:
                    ...
                    elastalert_logger.info("[elastalert.ElastAlerter.get_starttime()]Found expired previous run for %s at %s" % (rule['name'], endtime))
                    return None
        except (ElasticsearchException, KeyError) as e:
            elastalert_logger.error('[elastalert.ElastAlerter.get_starttime()]Error querying for last run: %s' % (e), {'rule': rule['name']},
                                    exc_info=True)
            self.handle_error('[elastalert.ElastAlerter.get_starttime()]Error querying for last run: %s' % (e), {'rule': rule['name']})
    ...
    def enhance_filter(self, rule):
        ...
        elastalert_logger.debug("[elastalert.ElastAlerter.enhance_filter()]Enhanced filter with {} terms: {}".format(listname, str(query_str_filter)))
    ...
    def run_rule(self, rule, endtime, starttime=None):
        ...
        if ts_now() <= rule['starttime']:
            elastalert_logger.warning("[elastalert.ElastAlerter.run_rule()]当前时间早于开始时间, starttime=(%s) > ts_now=(%s), \n\tso sleeping instead"
                                      % (starttime,ts_now()))
            return 0
        ...
        while rule['type'].matches:
            ...
            if self.is_silenced(rule['name'] + "._silence") or self.is_silenced(silence_cache_key):
                elastalert_logger.debug('[elastalert.ElastAlerter.run_rule()]Ignoring match for silenced rule= %s' % (silence_cache_key,))
                continue
                ...
                try:
                    for enhancement in rule['match_enhancements']:
                        try:
                            ...
                            elastalert_logger.error("[elastalert.ElastAlerter.run_rule()]Error running match enhancement: %s" % (e), {'rule': rule['name']},
                                                    exc_info=True)
                            self.handle_error("[elastalert.ElastAlerter.run_rule()]Error running match enhancement: %s" % (e), {'rule': rule['name']})
    ...
    def init_rule(self, new_rule, new=True):
        ...
        try:
            ...
            elastalert_logger.warning('[elastalert.ElastAlerter.init_rule()]Error connecting to Elasticsearch for rule {}. '
    ...
    def load_rule_changes(self):
        ...
        for rule_file, hash_value in self.rule_hashes.items():
            if rule_file not in new_rule_hashes:
                ...
                elastalert_logger.info('[elastalert.ElastAlerter.load_rule_changes()]no rulefile= %s, stop exec' % (rule_file))
            ...
            if hash_value != new_rule_hashes[rule_file]:
                ...
                try:
                    ...
                    if not new_rule:
                        elastalert_logger.error('[elastalert.ElastAlerter.load_rule_changes()]invalid rulefile= %s, skipped' % rule_file,
                                                exc_info=True)
                        continue
                    if 'is_enabled' in new_rule and not new_rule['is_enabled']:
                        elastalert_logger.info('[elastalert.ElastAlerter.load_rule_changes()]disabled rulefile= %s' % (rule_file))
                        # Remove this rule if it's been disabled
                        self.rules = [rule for rule in self.rules if rule['rule_file'] != rule_file]
                        continue
                except EAException as e:
                    message = 'Could not load rule= %s,error= %s' % (rule_file, e)
                    elastalert_logger.error('[elastalert.ElastAlerter.load_rule_changes()]'+message,
                                            exc_info=True)
                    self.handle_error('[elastalert.ElastAlerter.load_rule_changes()]'+message)
                    ...
                    continue
                elastalert_logger.info("[elastalert.ElastAlerter.load_rule_changes()]reloading rule= %s" % (rule_file))
            ...
            for rule_file in set(new_rule_hashes.keys()) - set(self.rule_hashes.keys()):
                try:
                    ...
                    if new_rule['name'] in [rule['name'] for rule in self.rules]:
                        elastalert_logger.error("[elastalert.ElastAlerter.load_rule_changes()]dupliacted rulename= %s"
                                                % (new_rule['name']),
                                                exc_info=True)
                        raise EAException("[elastalert.ElastAlerter.load_rule_changes()]dupliacted rulename= %s"
                                          % (new_rule['name']))
                except EAException as e:
                    elastalert_logger.error('[elastalert.ElastAlerter.load_rule_changes()]Could not load rule= %s,error= %s'
                                            % (rule_file, e),
                                            exc_info=True)
                    self.handle_error('[elastalert.ElastAlerter.load_rule_changes()]Could not load rule= %s,error= %s'
                                      % (rule_file, e))
                    self.send_notification_email(exception=e, rule_file=rule_file)
                    continue
                if self.init_rule(new_rule):
                    elastalert_logger.info('[elastalert.ElastAlerter.load_rule_changes()]Loaded new rule= %s' % (rule_file))
    ...
    def start(self):
        ...
        if self.starttime:
            if self.starttime == 'NOW':
                ...
                try:
                    ...
                    elastalert_logger.error("[elastalert.ElastAlerter.start()]starttime= %s, \n\tnot a valid ISO8601 timestamp (YYYY-MM-DDTHH:MM:SS+XX:00)"
                                            % (self.starttime),
                                            exc_info=True)
                    self.handle_error("[elastalert.ElastAlerter.start()]starttime= %s, \n\tnot a valid ISO8601 timestamp (YYYY-MM-DDTHH:MM:SS+XX:00)"
                                      % (self.starttime))
                    exit(1)
        ...
        self.running = True
        elastalert_logger.info("[elastalert.ElastAlerter.start()]Starting up")
        ...
        while self.running:
            ...
            if self.show_disabled_rules:
                elastalert_logger.info("[elastalert.ElastAlerter.start()]disabled rules= %s" % (str(self.get_disabled_rules())))
    ...
    def wait_until_responsive(self, timeout, clock=timeit.default_timer):
        ...
        if self.writeback_es.ping():
            ...
            elastalert_logger.error(
                '[elastalert.ElastAlerter.start()]failed to connect es= "%s:%d" ',
                self.conf['es_host'],
                self.conf['es_port'],
            )
        exit(1)
    ...
    def handle_pending_alerts(self):
        ...
        elastalert_logger.info("[elastalert.ElastAlerter.handle_pending_alerts()]后台警报%s在pretty(ts_now)= %s时\n\t发送待处理警报"
                               % (self.thread_data.alerts_sent, pretty_ts(ts_now())))

    def handle_config_change(self):
        if not self.args.pin_rules:
            self.load_rule_changes()
            elastalert_logger.info("[elastalert.ElastAlerter.handle_config_change()]在pretty(ts_now())= %s时\n\t检查后台配置是否改变"
                                   % (pretty_ts(ts_now()))
                                   )
    ...
    def handle_rule_execution(self, rule):
        ...
        try:
            ...
            elastalert_logger.error("[elastalert.ElastAlerter.handle_rule_execution()]Error running rule %s: %s" % (rule['name'], e), {'rule': rule['name']},
                                    exc_info=True)
            self.handle_error("[elastalert.ElastAlerter.handle_rule_execution()]Error running rule %s: %s" % (rule['name'], e), {'rule': rule['name']})
        except Exception as e:
            self.handle_uncaught_exception(e, rule)
        else:
            old_starttime = pretty_ts(rule.get('original_starttime'), rule.get('use_local_time'))
            elastalert_logger.info("[elastalert.ElastAlerter.handle_rule_execution()]rulename= %s;\n\tpretty(old_starttime)=\t%s;\n\tpretty(endtime)=\t%s;"
                                   "\n\tnum(hits): %s "
                                   "\n\tnum(already_seen): %s "
                                   "\n\tnum(matches): %s;"
                                   "\n\tnum(send_alerts)= %s"
            ...
            if next_run < datetime.datetime.utcnow():
                ...
                elastalert_logger.warning(
                    "[elastalert.ElastAlerter.handle_rule_execution()]下次跑批时间已晚于当前时间, "
                    "\n\tpretty(old_starttime)=\t%s;\n\tpretty(endtime)=\t%s;\n\trun_every=\t%s!" % (
                        old_starttime,
    ...
    def reset_rule_schedule(self, rule):
        ...
        if rule.get('limit_execution') and rule['next_starttime']:
            ...
            elastalert_logger.info('[elastalert.ElastAlerter.reset_rule_schedule()]暂停rulename= %s, '
                                   '\n\t下次启动时间pretty(next_starttime)= %s'
    ...
    def sleep_for(self, duration):
        ...
        elastalert_logger.info("[elastalert.ElastAlerter.sleep_for()]sleeping seconds= %s" % (duration))
        time.sleep(duration)
    ...
    def get_dashboard(self, rule, db_name):
        ...
        if res['hits']['hits']:
            ...
            elastalert_logger.error("[elastalert.ElastAlerter.get_dashboard()]Could not find dashboard named %s" % (db_name))
            raise EAException("[elastalert.ElastAlerter.get_dashboard()]Could not find dashboard named %s" % (db_name))
    ...
    def send_alert(self, matches, rule, alert_time=None, retried=False):
        ...
        if rule.get('generate_kibana_link') or rule.get('use_kibana_dashboard'):
            try:
                ...
                elastalert_logger.error("[elastalert.ElastAlerter.send_alert()]Could not generate Kibana dash for %s match: %s" % (rule['name'], e),exc_info=True)
                self.handle_error("[elastalert.ElastAlerter.send_alert()]Could not generate Kibana dash for %s match: %s" % (rule['name'], e))
            else:
                if kb_link:
            ...
            for enhancement in rule['match_enhancements']:
                ...
                for match in matches:
                    try:
                        ...
                        elastalert_logger.error("[elastalert.ElastAlerter.send_alert()]Error running match enhancement: %s"
                                                % (e), {'rule': rule['name']},exc_info=True)
                        self.handle_error("[elastalert.ElastAlerter.send_alert()]Error running match enhancement: %s"
                                          % (e), {'rule': rule['name']})
        ...
        for alert in rule['alert']:
            ...
            try:
                ...
                elastalert_logger.error('[elastalert.ElastAlerter.send_alert()]Error while running alert %s: %s'
                                        % (alert.get_info()['type'], e), {'rule': rule['name']},exc_info=True)
                self.handle_error('[elastalert.ElastAlerter.send_alert()]Error while running alert %s: %s'
    ...
    def writeback(self, doc_type, body, rule=None, match_body=None):
        ...
        if self.debug:
            elastalert_logger.info("[elastalert.ElastAlerter.writeback()]Skipping writing to ES: %s" % (writeback_body))
            return None
        ...
        try:
            ...
            elastalert_logger.exception("[elastalert.ElastAlerter.writeback()]Error writing alert info to Elasticsearch: %s" % (e),exc_info=True)
    ...
    def find_recent_pending_alerts(self, time_limit):
        ...
        try:
            ...
            elastalert_logger.exception("error= %s;query= %s" % (e, query),
                                        exc_info=True)
        return []
    ...
    def send_pending_alerts(self):
        ...
        for alert in pending_alerts:
            ...
            try:
                ...
                elastalert_logger.error('[elastalert.ElastAlerter.send_pending_alerts()]KeyError= %s' % (KeyError))
                continue
            # Find original rule
            for rule in self.rules:
            ...
            if ts_now() > ts_to_dt(alert_time):
                ...
                try:
                    ...
                    elastalert_logger.error("[elastalert.ElastAlerter.send_pending_alerts()]Failed to delete alert= %s;alert_time= %s"
                                            % (_id, alert_time),
                                            exc_info=True)
                    self.handle_error("[elastalert.ElastAlerter.send_pending_alerts()]Failed to delete alert %s at %s"
                                      % (_id, alert_time))
    ...
    def get_aggregated_matches(self, _id):
        ...
        try:
            ...
            elastalert_logger.error("[elastalert.ElastAlerter.get_aggregated_matches()]Error fetching aggregated matches: %s"
                                    % (e), {'id': _id},
                                    exc_info=True)
            self.handle_error("[elastalert.ElastAlerter.get_aggregated_matches()]Error fetching aggregated matches: %s"
                              % (e), {'id': _id})
        return matches
    ...
    def find_pending_aggregate_alert(self, rule, aggregation_key_value=None):
        ...
        try:
            ...
            elastalert_logger.error("[elastalert.ElastAlerter.find_pending_aggregate_alert()]Error searching for pending aggregated matches: %s"
                                    % (e), {'rule_name': rule['name']},exc_info=True)
            self.handle_error("[elastalert.ElastAlerter.find_pending_aggregate_alert()]Error searching for pending aggregated matches: %s" % (e), {'rule_name': rule['name']})
            return None
    ...
    def add_aggregated_alert(self, match, rule):
        ...
        if (not rule['current_aggregate_id'].get(aggregation_key_value) or
            ...
            if pending_alert:
                ...
                elastalert_logger.info(
                    '[elastalert.ElastAlerter.add_aggregated_alert()]Adding alert for rulename= %s to aggregation(id= %s, aggregation_key= %s), next alert at %s' % (
                        rule['name'],
                        agg_id,
                ...
                if isinstance(rule['aggregation'], dict) and rule['aggregation'].get('schedule'):
                    ...
                    try:
                        ...
                        elastalert_logger.error("[elastalert.ElastAlerter.add_aggregated_alert()]Error parsing aggregate send time Cron format %s"
                                                % (e), rule['aggregation']['schedule'],exc_info=True)
                        self.handle_error("[elastalert.ElastAlerter.add_aggregated_alert()]Error parsing aggregate send time Cron format %s" % (e), rule['aggregation']['schedule'])
                else:
                ...
                agg_id = None
                elastalert_logger.info(
                    '[elastalert.ElastAlerter.add_aggregated_alert()]New aggregation for %s, aggregation_key: %s. next alert at %s.'
            ...
            elastalert_logger.info(
                '[elastalert.ElastAlerter.add_aggregated_alert()]Adding alert for %s to aggregation(id: %s, aggregation_key: %s), next alert at %s' % (
                    rule['name'],
                    agg_id,
                    aggregation_key_value,
                    alert_time
    ...
    def silence(self, silence_cache_key=None):
        ...
        if self.debug:
            elastalert_logger.error('[elastalert.ElastAlerter.silence()]--silence not compatible with --debug')
            exit(1)

        if not self.args.rule:
            elastalert_logger.error('[elastalert.ElastAlerter.silence()]--silence must be used with --rule')
            exit(1)
        ...
        try:
            ...
            elastalert_logger.error('[elastalert.ElastAlerter.silence()]%s is not a valid time period' % (self.args.silence))
            exit(1)

        if not self.set_realert(silence_cache_key, silence_ts, 0):
            elastalert_logger.error('[elastalert.ElastAlerter.silence()]Failed to save silence command to Elasticsearch')
            exit(1)

        elastalert_logger.info('[elastalert.ElastAlerter.silence()]Success. %s will be silenced until %s' % (silence_cache_key, silence_ts))
    ...
    def is_silenced(self, rule_name):
        ...
        try:
            ...
            elastalert_logger.error("[elastalert.ElastAlerter.is_silenced()]Error while querying for alert silence status: %s" % (e), {'rule': rule_name},
                                    exc_info=True)
            self.handle_error("[elastalert.ElastAlerter.is_silenced()]Error while querying for alert silence status: %s" % (e), {'rule': rule_name})
    ...
    def handle_error(self, message, data=None):
        ...
        elastalert_logger.error('[elastalert.ElastAlerter.handle_error()]'+message)
    ...
    def handle_uncaught_exception(self, exception, rule):
        ...
        elastalert_logger.error('[elastalert.ElastAlerter.handle_uncaught_exception()]Uncaught exception running rule %s: %s'
                                % (rule['name'], exception), {'rule': rule['name']},
                                traceback.format_exc(),
                                exc_info=True)
        self.handle_error('[elastalert.ElastAlerter.handle_uncaught_exception()]Uncaught exception running rule %s: %s'
                          % (rule['name'], exception), {'rule': rule['name']})
        if self.disable_rules_on_error:
            ...
            elastalert_logger.info('[elastalert.ElastAlerter.handle_uncaught_exception()]disabledRule= %s ', rule['name'])
    ...
    def send_notification_email(self, text='', exception=None, rule=None, subject=None, rule_file=None):
        ...
        try:
            ...
            elastalert_logger.error('[elastalert.ElastAlerter.send_notification_email()]Error connecting to SMTP host: %s' % (e), {'email_body': email_body},
                                    exc_info=True)
            self.handle_error('[elastalert.ElastAlerter.send_notification_email()]Error connecting to SMTP host: %s' % (e), {'email_body': email_body})
...
def handle_signal(signal, frame):
    elastalert_logger.info('[elastalert.handle_signal()]elastalert接收到中断信号')
