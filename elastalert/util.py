...
def setlogger_byDefault(logfile_dir='logs/',logfile_name='run.log'):
    """
    set logger
    :param: logFile_dir 包含路径
    :param: logFile_name 文件名，带后缀
    """
    ...
    handler = RotatingFileHandler(
        ...
        maxBytes=1024*1024*1024,#单个文件1GB
        backupCount=10,#至多存10个文件
        encoding='utf8'
    )
...
elastalert_logger = setlogger_byDefault()
...
def get_module(module_name):
    ...
    try:
        ...
        elastalert_logger.error("[util.get_module()]Could not import module %s: %s" % (module_name, e)).with_traceback(sys.exc_info()[2],
                                                                                                                       exc_info=True)
...
def new_get_event_ts(ts_field):
    ...
    timestamp field.
    """
    res = lambda event: lookup_es_key(event[0], ts_field)
    return res
...
def dt_to_ts(dt):
    if not isinstance(dt, datetime.datetime):
        elastalert_logger.warning('[util.dt_to_ts()]Expected type: datetime, got type: %s' % (type(dt)))
        return dt
    ts = dt.isoformat()
...
def dt_to_ts_with_format(dt, ts_format):
    if not isinstance(dt, datetime.datetime):
        elastalert_logger.warning('[util.dt_to_ts_with_format()]Expected type= datetime, got type= %s' % (type(dt)))
        return dt
...
def build_es_conn_config(conf):
    ...
    #elastalert_logger.info('[util.build_es_conn_config()]parsed_conf='+str(parsed_conf))
    return parsed_conf
...
def getUrl():
    url_dict = {
        '40.2.214.10:50368':'http://40.2.214.10:50368/axis/services/AlarmSwitchService?wsdl',
        '40.2.214.12:50368':'http://40.2.214.12:50368/axis/services/AlarmSwitchService?wsdl',
        '197.3.134.69:58369':'http://197.3.134.69:58369/axis/services/AlarmSwitchService?wsdl'
    }
    ...
    for k,v in url_dict.items():
        ...
        ip = keyRes[0]
        port = keyRes[1]
        isOk = UrlOnService(ip, port)
        if(isOk):#url可用
            elastalert_logger.info('[util.getUrl()]可用的地址='+url_dict[k])
            return url_dict[k]


def send_WS(host, instance, parameter, Class, status, value, alarmTitle):
    """
    组装报文,初始化webservice客户端
    :param: host, instance, parameter, Class, status, value, alarmTitle, url
    """
    opDetail = {
        'host': host, 'instance': instance, 'parameter': parameter, 'class': Class, 'status': status, 'value': value,
        'max': '', 'min': '', 'occurtime': '', 'alarmId': '', 'appSystem': 'ELK日志系统', 'alarmTitle': alarmTitle
    }
    xmlContent_body = []
    for k in opDetail.keys():
        v = opDetail.get(k)
        xmlContent_body.append(
    ...
    imp.filter.add('urn:AlarmSwitchService')
    d = ImportDoctor(imp)
    d.add(Import('http://schemas.xmlsoap.org/soap/encoding/'))
    d.add(Import('http://schemas.xmlsoap.org/wsdl/soap/'))
    d.add(Import('http://schemas.xmlsoap.org/wsdl/'))
    d.add(Import('http://xml.apache.org/xml-soap'))
    wsClient = Client(url, doctor=d)
    try:
        responseResult = wsClient.service.transferAlarm('', '', '', '', xmlContent)
        responseResult_list = re.split(r';',str(responseResult))
        status = responseResult_list[0]
        error_list= responseResult_list[1]
    except Exception as e:
        elastalert_logger.error('[util.send_WS()]error= '+e+'\n error_list='+str(error_list))
    elastalert_logger.info('[util.send_WS()]sended_status= %s' %(status))

def filter_special_characters(value_str):
    value_str = value_str.replace('\'','&apos')
    value_str = value_str.replace('\"','&quot')
    value_str = value_str.replace('<','&lt')
    value_str = value_str.replace('>','&gt')
    return value_str
#fyq added
