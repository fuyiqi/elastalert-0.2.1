    ...
    def load_alerts(self, rule, alert_field):
        ...
        def create_alert(alert, alert_config):
            ...
            if not issubclass(alert_class, alerts.Alerter):
                elastalert_logger.error('[loaders.create_alert()]Alert module %s is not a subclass of Alerter' % alert,
                                        exc_info=True
                                        )
            ...
            missing_options = (rule['type'].required_options | alert_class.required_options) - frozenset(
                alert_config or [])
            if missing_options:
                elastalert_logger.error('[loaders.create_alert()]Missing required option(s): %s' % (', '.join(missing_options)),
                                        exc_info=True)
        ...
        try:
            ...
            elastalert_logger.error('[loaders.create_alert()]Error initiating alert %s: %s' % (rule['alert'], e),
    ...
    def adjust_deprecated_values(rule):
        ...
        if rule.get('type') == 'simple':
            ...
            elastalert_logger.warning(
                '[loaders.adjust_deprecated_values()]"simple" alerter has been renamed "post" and comptability may be removed in a future release.')
...
class FileRulesLoader(RulesLoader):
    ...
    def get_yaml(self, filename):
        try:
            ...
            elastalert_logger.error('[loaders.get_yaml()]Could not parse file %s: %s' % (filename, e),
                                    exc_info=True)
