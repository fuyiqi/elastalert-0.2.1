...
from socket import error, if_indextoname
    ...
    def custom_alert_text(self):
        ...
        if 'alert_text_args' in self.rule:
            ...
            for name, kw_name in list(self.rule.get('alert_text_kw').items()):
                ...
                kw[kw_name] = missing if val is None else val
            elastalert_logger.info("[BasicMatchString.custom_alert_text()]raw alert_text="+alert_text+"===================")#缺内容的壳子
            elastalert_logger.info("[BasicMatchString.custom_alert_text()]kw="+str(kw)+"\n=======================")#壳子的内容
            alert_text = alert_text.format(**kw)
        ...
        for match in matches:
            ...
            if resmatch is not None:
                elastalert_logger.info(
                    '[alerts.StompAlerter.alert()]Alert for %s, %s at %s:' % (self.rule['name'], resmatch, lookup_es_key(match, self.rule['timestamp_field'])))
                alerts.append(
                ...
                elastalert_logger.info('[alerts.StompAlerter.alert()]Rule %s generated an alert at %s:' % (
                ...
                fullmessage['match'] = lookup_es_key(
                    match, self.rule['timestamp_field'])
            elastalert_logger.info('[alerts.StompAlerter.alert()]'+str(BasicMatchString(self.rule, match)))
...
class EmailAlerter(Alerter):
    ...
    def alert(self, matches):
        ...
        try:
            ...
            elastalert_logger.error("[alerts.EmailAlerter.alert()]Error connecting to SMTP host: %s" % (e))
            raise EAException("[alerts.EmailAlerter.alert()]Error connecting to SMTP host: %s" % (e))
        except SMTPAuthenticationError as e:
            elastalert_logger.error("[alerts.EmailAlerter.alert()]SMTP username/password rejected: %s" % (e))
            raise EAException("[alerts.EmailAlerter.alert()]SMTP username/password rejected: %s" % (e))
        self.smtp.sendmail(self.from_addr, to_addr, email_msg.as_string())
        self.smtp.quit()

        elastalert_logger.info("[alerts.EmailAlerter.alert()]Sent email to %s" % (to_addr))
...
class JiraAlerter(Alerter):
    ...
    def __init__(self, rule):
        ...
        if self.bump_in_statuses and self.bump_not_in_statuses:
            ...
            elastalert_logger.warning('[alerts.JiraAlerter.__init__()]'+msg)
        ...
        try:
            ...
            elastalert_logger.error("[alerts.JiraAlerter.__init__()]Error connecting to JIRA: %s" % (str(e)[:1024]),
                                    exc_info=True).with_traceback(sys.exc_info()[2])
            raise EAException("[alerts.JiraAlerter.__init__()]Error connecting to JIRA: %s" % (str(e)[:1024])).with_traceback(sys.exc_info()[2])
    ...
    def set_priority(self):
        try:
            ...
            elastalert_logger.error("[alerts.JiraAlerter.__init__()]Priority %s not found. Valid priorities are %s" % (self.priority, list(self.priority_ids.keys())),
                                    exc_info=True)
    ...
    def set_jira_arg(self, jira_field, value, fields):
        ...
        if not field:
            ...
            elastalert_logger.error("[alerts.JiraAlerter.set_jira_arg()]Could not find a definition for the jira field '{0}'".format(normalized_jira_field),
                                    exc_info=True)
            raise Exception("[alerts.JiraAlerter.set_jira_arg()]Could not find a definition for the jira field '{0}'".format(normalized_jira_field))
        arg_name = field['id']
        ...
        if not ('schema' in field or 'type' in field['schema']):
            elastalert_logger.error("[alerts.JiraAlerter.set_jira_arg()]Could not determine schema information for the jira field '{0}'".format(normalized_jira_field),
                                    exc_info=True)
            raise Exception("[alerts.JiraAlerter.set_jira_arg()]Could not determine schema information for the jira field '{0}'".format(normalized_jira_field))
    ...
    def find_existing_ticket(self, matches):
        ...
        try:
            ...
            elastalert_logger.exception("[alerts.JiraAlerter.find_existing_ticket()]Error while searching for JIRA ticket using jql '%s': %s" % (jql, e),
                                        exc_info=True)
            return None
        ...
        if self.bump_tickets:
            ...
            if ticket:
                ...
                if ts_to_dt(ticket.fields.updated) >= inactivity_datetime:
                    ...
                    return None
                elastalert_logger.info('[alerts.JiraAlerter.alert()]Commenting on existing ticket %s' % (ticket.key))
                for match in matches:
                    try:
                        self.comment_on_ticket(ticket, match)
                    except JIRAError as e:
                        elastalert_logger.exception("[alerts.JiraAlerter.alert()]Error while commenting on ticket %s: %s" % (ticket, e),
                                                    exc_info=True)
                    if self.labels:
                        for l in self.labels:
                            try:
                                ...
                                elastalert_logger.exception("[alerts.JiraAlerter.alert()]Error while appending labels to ticket %s: %s" % (ticket, e),
                                                            exc_info=True)
                if self.transition:
                    elastalert_logger.info('[alerts.JiraAlerter.alert()]Transitioning existing ticket %s' % (ticket.key))
                    try:
                        self.transition_ticket(ticket)
                    except JIRAError as e:
                        elastalert_logger.exception("[alerts.JiraAlerter.alert()]Error while transitioning ticket %s: %s" % (ticket, e),
                                                    exc_info=True)
            ...
            if self.watchers:
                for watcher in self.watchers:
                    try:
                        ...
                        elastalert_logger.error("[alerts.JiraAlerter.alert()]Exception encountered when trying to add '{0}' as a watcher. Does the user exist?\n{1}" .format(
                                watcher,
                                ex
                            ),exc_info=True).with_traceback(sys.exc_info()[2])
                        raise Exception(
                            "[alerts.JiraAlerter.alert()]Exception encountered when trying to add '{0}' as a watcher. Does the user exist?\n{1}" .format(
                                watcher,
                                ex
                            )).with_traceback(sys.exc_info()[2])

        except JIRAError as e:
            elastalert_logger.error("[alerts.JiraAlerter.alert()]Error creating JIRA ticket using jira_args (%s): %s" % (self.jira_args, e),
                                    exc_info=True)
            raise EAException("[alerts.JiraAlerter.alert()]Error creating JIRA ticket using jira_args (%s): %s" % (self.jira_args, e))
        elastalert_logger.info("[alerts.JiraAlerter.alert()]Opened Jira ticket: %s" % (self.issue))
...
class CommandAlerter(Alerter):
    ...
    def alert(self, matches):
        ...
        try:
            ...
            elastalert_logger.info("[CommandAlerter.alert()]isHostValid=%s,;isIPValid=%s",str(isHostValid),str(isIPValid))
            if(isIPValid): # ip可用
                host = cat_dict['ip']
            else: #ip不可用
            ...
            send_WS(host=host,instance=instance,parameter=parameter,Class=Class,status=status,value=value,alarmTitle=alarmTitle)
...
class AlertaAlerter(Alerter):
    ...
    def __init__(self, rule):
        ...
        self.missing_text = self.rule.get('alert_missing_value', 'MISSING_VALUE')
